...
%       Default cutMask: Pixels that are less than 16 pixels away from the
...
function [result, mask] = getInPoints( edgeMap, cutMask, thresholded )

    if( ~exist( 'cutMask', 'var' ) || isempty( cutMask ) )
        cutMask = false( size( edgeMap ) );
        cutMask( 1: 15, : ) = true;
        cutMask( end - 15: end, : ) = true;
        cutMask( :, 1: 15 ) = true;
        cutMask( :, end - 15: end ) = true;
    end
    ...
    if( ~exist( 'thresholded', 'var' ) || isempty( thresholded ) || ...
        ...
        mask( edgeMap > 0.5 ) = 1;   % 阈值分割，大于0.5的为运动轮廓
    else
        mask = edgeMap;
    end

    mask = uint8( cleanBoundaries( logical( mask ), cutMask ) ); % 四周15个像素外，置为0

    result = integralIntersections( mask );
    % A point on a boundary cannot be "in"
    result( logical( mask ) ) = 0;  % 去掉轮廓点，得到的只是内部
    mask =  logical( mask );
end
