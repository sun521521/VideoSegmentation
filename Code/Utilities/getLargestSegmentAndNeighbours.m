...
function output = getLargetSegmentAndNeighboursInFrame( map, ratio )
    ...
    for( i = 1: labels )
        if( segments(i).Area > largestSize )
            largestSize = segments(i).Area;   %  找最大值
            largest = i;
        end
    end
    
    % Expand largest segment
    best = labelmap == largest;  % 二值图像
    disk = strel( 'disk', round( ratio * sqrt( largestSize / pi ) ), 0);  % 模板
    best = imdilate( best, disk );  % imdilate  灰度膨胀
