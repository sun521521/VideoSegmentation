...
function flow = computeBroxPAMI2011Flow( options, shot )
    ...
    endFrame = options.ranges( shot + 1 )-1 ;  %%%%  -1
    ...
    currImage = readFrame( options, startFrame );  % 另外包含：伪彩色 + 重设图片大小
    ...
    for( i = startFrame + 1: endFrame )
        ...
        if( options.vocal )
            fprintf( 'computeBroxPAMI2011Flow: Computing optical flow of pair: %d of %d... ', ...
                index, flowframes );
        end

        flowframe = int16( mex_LDOF( currImage, nextImage ) );  %  输入彩色图像， double。 输出 H x W x 2，下面为 u
