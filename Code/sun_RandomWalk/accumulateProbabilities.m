...
function [ result, probabilities ] = accumulateProbabilities( params, data, probabilities, bounds )
    
    if( exist( 'params', 'var' ) && isfield( params, 'lambda' ) )
        lambda = params.lambda;
    else
        lambda = 2;   % 5     %  可信程度 lanmda
    end
    
    flow = data.flow;
    superpixels = data.superpixels;
%     map = data.inMaps;
    % connectivity：13 x 1 cell.每个cell为一个邻接矩阵，前一帧与后一帧的超像素块，相交的像素数
...
%     spixelRatio = getSuperpixelInRatio( superpixels, map );  % 在前面的videoRapidSegment中，已经计算过
    ...
    spixelProb = cell( frames, 1 );
    for i = 1:frames
        spixelProb{i} = probabilities( bounds(i):bounds(i+1)-1, 1 );
    end
    ...
    output{ 1 } = double( spixelProb{ 1 }' );   % 1x 1358
    foreGain{ 1 } = zeros( length( spixelProb{ 1 } ), 1 ); % 1358 x 1
...
%           % 修改
%         [inSegments exSegments] = sun_getInExSegments(  spixelProb{frame}, superpixels{frame}, data.edgePoints{frame}, 0.2, 0.3);
...
%         alpha = double( exp( - lambda * alpha ) );
    ...
    output = cell( frames, 1 );
    output{ frames } = double( spixelProb{ frames } );
    backGain{ frames } = zeros( length( spixelProb{ frames } ), 1 );
...
%         alpha = double( exp( - lambda * alpha ) );
        ...
        output{ frame } = backGain{ frame } + double( spixelProb{ frame } );
    ...
    for( i = 1: frames )
        tags{ i } = spixelProb{ i } + 0.7*foreGain{ i } + 0.7*backGain{ i };
        
        result{ i } = tags{ i }( superpixels{ i } );
    end
    
    probabilities = cell2mat(tags);
end
